...
until mysqladmin ping -h "$WORDPRESS_DB_HOST" -u root -p"$MYSQL_ROOT_PASSWORD" --protocol=TCP --ssl=FALSE --silent 2>/dev/null; do
...
if [ "$TABLE_COUNT" = "0" ] || [ -z "$TABLE_COUNT" ]; then
  ...
  mysql -h "$WORDPRESS_DB_HOST" -u "$WORDPRESS_DB_USER" -p"$WORDPRESS_DB_PASSWORD" "$WORDPRESS_DB_NAME" \
    --protocol=TCP --ssl=FALSE < /docker-entrypoint-initdb.d/init.sql
